...
* **Explorer right-click "MT DevOps: Index Repo (force reindex)" / "MT DevOps: Update Repo Index (fill gaps only)"** -- the Repo Hub tree's own "Index This Repo"/"Update This Repo" actions, reachable from wherever a repo is already open in the editor (matches by folder basename against `mt-hub`'s own `-r`/`--repo` filter, so right-click the repo's root folder, not a file inside it).
