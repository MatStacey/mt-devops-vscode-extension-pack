...
* **Explorer right-click "MT DevOps: Update Repo Index (fill gaps only)"** -- the Repo Hub tree's own "Update This Repo" action, reachable from wherever a repo is already open in the editor (matches by folder basename against `mt-hub`'s own `-r`/`--repo` filter, so right-click the repo's root folder, not a file inside it).
